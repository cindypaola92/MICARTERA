...
function saldoCuota(p,i){const a=(p.abonos||[])[i];const m=a?a.monto:null;return m==null?p.cuota:Math.max(0,p.cuota-m);}
...
function todosLosRecaudos(prestamos){const o=[];(prestamos||[]).forEach(p=>{const fs=fechasCuotas(p);(p.abonos||[]).forEach((a,i)=>{if(a&&a.monto>0&&a.fecha){o.push({p,i,monto:a.monto,fecha:parseD(a.fecha),fCuota:i<fs.length?fs[i]:parseD(a.fecha)});}});});return o.sort((a,b)=>b.fecha-a.fecha);}
function estadoCuota(p,i,hoy){const h=hoy||HOY();const fs=fechasCuotas(p);const a=(p.abonos||[])[i];const m=a?a.monto:null;if(m==null){if(!fs[i]||fs[i]>h)return 'futuro';return 'sinpago';}if(m===0)return 'sinpago';if(m>=p.cuota-0.5)return 'completa';return 'parcial';}
...
function ModalCliente({p,onClose}){
  ...
  const nS=Math.max(10,(p.abonos||[]).length);
  const absHist=(p.abonos||[]).filter(a=>a&&a.monto>0&&a.fecha).sort((a,b)=>parseD(b.fecha)-parseD(a.fecha));
      ...
      <div style={{overflowX:'auto'}}>
        <table style={{width:'100%',borderCollapse:'collapse',fontSize:12,minWidth:460}}>
          ...
          <tbody>{Array.from({length:nS},(_,i)=>{
            const f=i<fs.length?fs[i]:null;const a=(p.abonos||[])[i];const m=a?a.monto:null;const fa=a&&a.fecha?a.fecha:null;
...
function CobrosView({prestamos,onSave,semOff,setSemOff,currentUser,onVerCliente}){
  ...
  const cobros=useMemo(()=>{const out=[];prestamos.filter(p=>!isTerminado(p)).forEach(p=>{if(fr&&p.ruta!==fr)return;if(!rutasCob.includes(p.ruta))return;const cuotas=fechasCuotas(p);let tiene=false;cuotas.forEach((f,i)=>{if(enRango(f,lun,dom)){out.push({p,i,f,ext:false});tiene=true;}});if(!tiene&&saldoTotal(p)>0.5&&lun>cuotas[cuotas.length-1])out.push({p,i:(p.abonos||[]).length,f:lun,ext:true});});return out;},[prestamos,semOff,fr,currentUser]);
  ...
  return <div>
    ...
    {cobros.length===0?<Empty msg="Sin cobros esta semana."/>:(fr?[fr]:rutasCob).map(k=>{
      ...
      const espK=xs.reduce((s,x)=>s+x.p.cuota,0);const recK=xs.reduce((s,x)=>s+((x.p.abonos||[])[x.i]?((x.p.abonos||[])[x.i].monto||0):0),0);
