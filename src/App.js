    ...
    <div style={{padding:'16px 20px'}}>
      ...
      <div style={{overflowX:'auto'}}>
        <table style={{width:'100%',borderCollapse:'collapse',fontSize:12,minWidth:460}}>
          <thead><tr>{['Cuota','Fecha vence','Pactado','Abonado','Fecha de pago','Estado','Saldo'].map((t,i)=><th key={i} style={{textAlign:'left',padding:'5px 7px',fontSize:10,color:MT,fontWeight:700,borderBottom:'2px solid '+BD}}>{t}</th>)}</tr></thead>
          <tbody>{Array.from({length:nS},(_,i)=>{
            ...
            const esExt=i>=10;const s=saldoCuota(p,i);
            let bg='transparent',ec=FT,el='Pendiente';
            if(m!=null&&m>0&&m>=p.cuota-0.5){bg=OKB+'66';ec=OK;el='Completa';}
            else if(m!=null&&m>0){bg=WNB+'66';ec=WN;el='Parcial';}
            else if(m===0){bg=DNB+'66';ec=DN;el='Sin pago';}
            else if(f&&f<h){bg=DNB+'33';ec=DN;el='No registrado';}
            return <tr key={i} style={{borderBottom:'1px solid '+BD,background:bg}}>
              ...
              <td style={{padding:'6px 7px',fontSize:11,color:m&&m>0?G:MT,fontWeight:m&&m>0?600:400}}>{fa?fmtFL(parseD(fa)):'-'}</td>
...
function ModalSemana({lun,dom,rutaInicial,prestamos,onClose,onVerCliente}){
  ...
  return <Overlay onClose={onClose}>
    ...
    <div style={{padding:'0 18px',borderBottom:'1px solid '+BD,display:'flex',gap:4,paddingTop:8}}>
      {tabs.map(t=><button key={t.v} onClick={()=>setRutaTab(t.v)} style={{padding:'6px 12px',border:'none',borderBottom:rutaTab===t.v?'2px solid '+G:'2px solid transparent',background:'none',cursor:'pointer',fontSize:13,fontWeight:rutaTab===t.v?700:400,color:rutaTab===t.v?G:MT,fontFamily:'inherit'}}>{t.l}</button>)}
    </div>
    <div style={{padding:'14px 18px'}}>
      <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:9,marginBottom:16}}>
        <KPI label="Recaudado" value={fmt(tR)} vc={OK} sub={recs.length+' abonos'}/>
        <KPI label="Prestado nuevo" value={fmt(tP)} sub={nuevos.length+' clientes'}/>
        <KPI label="Flujo neto" value={(tR-tP>=0?'+':'')+fmt(tR-tP)} vc={tR-tP>=0?OK:DN}/>
        <KPI label="Clientes nuevos" value={String(nuevos.length)} sub={rutaTab==='todas'?'todas las rutas':'Ruta '+rutaTab}/>
      </div>
      {recs.length>0&&<div style={{marginBottom:16}}>
        <div style={{fontSize:13,fontWeight:700,marginBottom:8}}>Cobros recibidos ({recs.length})</div>
        <div style={{overflowX:'auto'}}><table style={{width:'100%',borderCollapse:'collapse',fontSize:13,minWidth:480}}>
          ...
          <tbody>{recs.map((x,i)=><tr key={i} style={{borderBottom:'1px solid '+BD,background:i%2===0?'transparent':S2}}>
            <td style={{padding:'7px 8px',fontWeight:600,cursor:'pointer',color:G}} onClick={()=>{onClose();onVerCliente(x.p);}}>{x.p.nombre}</td>
            ...
            <td style={{padding:'7px 8px',fontWeight:600}}>{x.i+1>10?'C'+(x.i+1)+' *':'C'+(x.i+1)}</td>
      ...
      {nuevos.length>0&&<div>
        ...
        <div style={{overflowX:'auto'}}><table style={{width:'100%',borderCollapse:'collapse',fontSize:13,minWidth:440}}>
          ...
          <tbody>{nuevos.map((p,i)=><tr key={i} style={{borderBottom:'1px solid '+BD,background:i%2===0?'transparent':S2}}>
            <td style={{padding:'7px 8px',fontWeight:600,cursor:'pointer',color:G}} onClick={()=>{onClose();onVerCliente(p);}}>{p.nombre}</td>
      ...
      {recs.length===0&&nuevos.length===0&&<Empty msg="Sin movimiento esta semana en esta ruta"/>}
    </div>
  </Overlay>;
}
        ...
        return <div key={i} onClick={()=>{onClose();onSelectCliente(p);}} style={{padding:'10px 4px',borderBottom:'1px solid '+BD,cursor:'pointer',borderRadius:6}} onMouseEnter={e=>e.currentTarget.style.background=S2} onMouseLeave={e=>e.currentTarget.style.background='transparent'}>
          ...
          {cuotasCaidas.length>0&&<div style={{display:'flex',flexWrap:'wrap',gap:4,paddingLeft:4,marginBottom:2}}>
            {cuotasCaidas.map((c,j)=><span key={j} style={{display:'inline-flex',gap:3,background:DNB,color:DN,borderRadius:5,padding:'2px 8px',fontSize:11,fontWeight:600}}>
              {c.idx+1>10?'C'+(c.idx+1)+' *':'C'+(c.idx+1)}{c.abMonto>0?' (abono '+fmt(c.abMonto)+')':''}: {fmt(c.saldo)}
            </span>)}
          </div>}
        </div>;
      })}
...
function Dashboard({prestamos,salidas,flujoOff,setFlujoOff,onVerCliente}){
  ...
  const coEsta=[];activos.forEach(p=>{const fs=fechasCuotas(p);let tiene=false;fs.forEach((f,i)=>{if(enRango(f,lh,dh)){coEsta.push({p,i});tiene=true;}});if(!tiene&&saldoTotal(p)>0.5&&fs[9]<lh){coEsta.push({p,i:getAbonos(p).length,ext:true});}});
  const esp=coEsta.filter(x=>!x.ext).reduce((s,x)=>s+x.p.cuota,0);
  ...
  const flujo=useMemo(()=>{const o={};['A','B','C'].forEach(r=>{o[r]={rec:0,nAb:0,cap:0,nPr:0};});recs.forEach(rc=>{if(enRango(rc.fecha,fL,fD)){o[rc.p.ruta].rec+=rc.monto;o[rc.p.ruta].nAb++;}});prestamos.forEach(p=>{if(enRango(parseD(p.fechaPrestamo),fL,fD)){o[p.ruta].cap+=p.monto;o[p.ruta].nPr++;}});return o;},[prestamos,flujoOff]);
  const HIST=12;
  ...
  return <div>
    ...
    <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:10,marginBottom:14}}>
      {['A','B','C'].map(k=>{const ps=activos.filter(p=>p.ruta===k);const sl=ps.reduce((s,p)=>s+saldoTotal(p),0);const at=ps.reduce((s,p)=>s+saldoAtrasado(p),0);const conAtR=ps.filter(tieneSaldoAtrasado).length;return <div key={k} style={{background:SF,border:'1px solid '+RUTAS[k].c+'44',borderRadius:10,padding:'12px 14px'}}>
        <div style={{display:'flex',alignItems:'center',gap:6,marginBottom:8}}><RTag r={k}/><span style={{fontSize:11,color:MT}}>{RUTAS[k].cobrador} - {RUTAS[k].dia}</span></div>
        <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:6}}>
          <div style={{cursor:'pointer'}} onClick={()=>setMLista({titulo:'Ruta '+k+' - Clientes',lista:[...ps].sort((a,b)=>saldoTotal(b)-saldoTotal(a)),campo:'saldo'})}><div style={{fontSize:10,color:MT,marginBottom:2}}>Clientes</div><div style={{fontSize:16,fontWeight:700,color:RUTAS[k].c}}>{ps.length}</div></div>
          <div style={{cursor:'pointer'}} onClick={()=>setMLista({titulo:'Ruta '+k+' - Por cobrar',lista:[...ps].sort((a,b)=>saldoTotal(b)-saldoTotal(a)),campo:'saldo'})}><div style={{fontSize:10,color:MT,marginBottom:2}}>Por cobrar</div><div style={{fontSize:14,fontWeight:700,color:G}}>{fmt(sl)}</div></div>
          <div style={{cursor:'pointer'}} onClick={()=>setMLista({titulo:'Ruta '+k+' - Saldo atrasado',lista:ps.filter(tieneSaldoAtrasado).sort((a,b)=>saldoAtrasado(b)-saldoAtrasado(a)),campo:'at'})}><div style={{fontSize:10,color:MT,marginBottom:2}}>Atrasado</div><div style={{fontSize:14,fontWeight:700,color:at>0?DN:FT}}>{at>0?fmt(at):'-'}</div><div style={{fontSize:10,color:FT}}>{conAtR} cli.</div></div>
        </div>
      </div>;})}
    </div>
    ...
    <Panel title={'Historial - ultimas '+HIST+' semanas'} tr={<span style={{fontSize:11,color:G,fontWeight:600}}>Clic en una celda para ver detalle</span>}>
      <div style={{overflowX:'auto'}}><table style={{width:'100%',borderCollapse:'collapse',fontSize:12,minWidth:900}}>
        <thead>
          ...
          <tr>
            {['A','B','C'].map(k=>[
              <th key={k+'cli'} style={{padding:'3px 6px',fontSize:9,color:RUTAS[k].c,fontWeight:700,borderBottom:'2px solid '+BD,borderLeft:'2px solid '+RUTAS[k].c,textAlign:'center',background:RUTAS[k].bg}}>Cli. nuevos</th>,
        ...
        <tbody>{hist.map((x,idx)=>{
          ...
          return <tr key={idx} style={{borderBottom:'1px solid '+BD,background:esA?GB:'transparent'}}>
            ...
            {['A','B','C'].map(k=>[
              <td key={k+'cli'} style={{padding:'6px',textAlign:'center',borderLeft:'2px solid '+RUTAS[k].c,cursor:'pointer'}} onClick={()=>setMSemana({lun:x.lun,dom:x.dom,ruta:k})} onMouseEnter={e=>e.currentTarget.style.background=RUTAS[k].bg} onMouseLeave={e=>e.currentTarget.style.background='transparent'}><span style={{fontWeight:700,color:x.f[k].nPr?RUTAS[k].c:FT}}>{x.f[k].nPr||'-'}</span></td>,
              <td key={k+'rec'} style={{padding:'6px',textAlign:'center',cursor:'pointer'}} onClick={()=>setMSemana({lun:x.lun,dom:x.dom,ruta:k})} onMouseEnter={e=>e.currentTarget.style.background=RUTAS[k].bg} onMouseLeave={e=>e.currentTarget.style.background='transparent'}><span style={{fontWeight:700,color:x.f[k].rec?OK:FT}}>{x.f[k].rec?fmt(x.f[k].rec):'-'}</span></td>,
              <td key={k+'pre'} style={{padding:'6px',textAlign:'center',cursor:'pointer'}} onClick={()=>setMSemana({lun:x.lun,dom:x.dom,ruta:k})} onMouseEnter={e=>e.currentTarget.style.background=RUTAS[k].bg} onMouseLeave={e=>e.currentTarget.style.background='transparent'}><span style={{color:x.f[k].cap?TX:FT}}>{x.f[k].cap?fmt(x.f[k].cap):'-'}</span></td>
            ])}
...
function CobrosView({prestamos,onSave,semOff,setSemOff,currentUser,onVerCliente}){
  ...
  return <div>
    ...
    {cobros.length===0?<Empty msg="Sin cobros esta semana."/>:(fr?[fr]:rutasCob).map(k=>{
      ...
      const espK=xs.filter(x=>!x.ext).reduce((s,x)=>s+x.p.cuota,0);
      const recK=xs.reduce((s,x)=>s+(getAbonos(x.p)[x.i]?(getAbonos(x.p)[x.i].monto||0):0),0);
      return <Panel key={k} title={<span><RTag r={k}/><CTag r={k}/><span style={{fontSize:12,color:MT,marginLeft:7}}>{RUTAS[k].dia} - {xs.length} cobros</span></span>} tr={<span style={{fontSize:12,color:MT}}>Recogido <b style={{color:OK}}>{fmt(recK)}</b> de {fmt(espK)}</span>}>
        <div style={{overflowX:'auto'}}><table style={{width:'100%',borderCollapse:'collapse',fontSize:13,minWidth:620}}>
          ...
          <tbody>{xs.map((x,idx)=>{
            const m=getAbonos(x.p)[x.i]?getAbonos(x.p)[x.i].monto:null;
            const fa=getAbonos(x.p)[x.i]?getAbonos(x.p)[x.i].fecha:null;
            const saldo=x.ext?saldoTotal(x.p):saldoCuota(x.p,x.i);
            const rowBg=x.ext?DNB+'44':undefined;
            return <tr key={idx} style={{borderBottom:'1px solid '+BD,background:rowBg}}>
              ...
              <td style={{padding:'8px'}}><b>{x.ext?'C'+(x.i+1)+' *':'C'+(x.i+1)}</b></td>
              ...
              <td style={{padding:'8px'}}>
                <div style={{display:'flex',gap:3}}>
                  <Btn sm v="primary" onClick={()=>onSave(x.p.id,x.i,'completo',toDS(x.f),null,x.ext)}>OK</Btn>
                  {!x.ext&&<Btn sm style={{color:WN,borderColor:WN+'44'}} onClick={()=>onSave(x.p.id,x.i,0,toDS(x.f))}>X</Btn>}
                  {m!=null&&<Btn sm onClick={()=>onSave(x.p.id,x.i,'limpiar')}>Borrar</Btn>}
                </div>
              </td>
            </tr>;
          })}</tbody>
...
function ConsolidadoView({prestamos,onEdit,onDelete,currentUser,onVerCliente}){
  const [rf,setRf]=useState('');const [estado,setEstado]=useState('activo');const [nombre,setNombre]=useState('');const [filtCuotas,setFiltCuotas]=useState('');const [filtPagas,setFiltPagas]=useState('');
  const rutasCob=currentUser.rol==='cobrador'?(currentUser.rutas||[currentUser.ruta]):null;
  function cuotasLlevadas(p){const h=HOY();return fechasCuotas(p).filter(f=>f<=h).length;}
  function cuotasPagas(p){return getAbonos(p).filter(a=>a&&a.monto>0).length;}
  const lista=useMemo(()=>{let l=prestamos.slice();if(rf)l=l.filter(p=>p.ruta===rf);if(estado==='activo')l=l.filter(p=>!isTerminado(p));else if(estado==='saldo')l=l.filter(p=>!isTerminado(p)&&tieneSaldoAtrasado(p));else if(estado==='terminado')l=l.filter(isTerminado);if(nombre)l=l.filter(p=>p.nombre.toLowerCase().includes(nombre.toLowerCase()));if(rutasCob)l=l.filter(p=>rutasCob.includes(p.ruta));if(filtCuotas!=='')l=l.filter(p=>cuotasLlevadas(p)===parseInt(filtCuotas));if(filtPagas!=='')l=l.filter(p=>cuotasPagas(p)===parseInt(filtPagas));return l.sort((a,b)=>parseD(b.fechaPrestamo)-parseD(a.fechaPrestamo));},[prestamos,rf,estado,nombre,currentUser,filtCuotas,filtPagas]);
  ...
  return <div>
    ...
    <div style={{display:'flex',gap:8,marginBottom:12,flexWrap:'wrap',alignItems:'flex-end'}}>
      ...
      <Sel value={filtCuotas} onChange={e=>setFiltCuotas(e.target.value)} style={{maxWidth:160}}><option value="">Todas las cuotas</option>{Array.from({length:10},(_,i)=><option key={i+1} value={i+1}>{i+1} cuota{i+1!==1?'s':''} llevadas</option>)}</Sel>
      <Sel value={filtPagas} onChange={e=>setFiltPagas(e.target.value)} style={{maxWidth:160}}><option value="">Todas las pagadas</option>{Array.from({length:11},(_,i)=><option key={i} value={i}>{i} cuota{i!==1?'s':''} pagas</option>)}</Sel>
      {(filtCuotas!==''||filtPagas!=='')&&<button onClick={()=>{setFiltCuotas('');setFiltPagas('');}} style={{padding:'4px 10px',border:'1px solid '+BDS,borderRadius:7,background:SF,color:MT,cursor:'pointer',fontSize:12,fontFamily:'inherit'}}>Limpiar filtros</button>}
      <span style={{fontSize:12,color:FT}}>{lista.length} prestamo{lista.length!==1?'s':''}{filtCuotas!==''&&filtPagas!==''?<b style={{color:G}}> con {filtCuotas} cuotas y {filtPagas} pagas</b>:filtCuotas!==''?<b style={{color:G}}> con {filtCuotas} cuotas llevadas</b>:filtPagas!==''?<b style={{color:G}}> con {filtPagas} cuotas pagas</b>:null}</span>
    </div>
    {lista.length===0?<Empty msg="Sin resultados."/>:<div style={{background:SF,border:'1px solid '+BD,borderRadius:10,overflowX:'auto',marginBottom:12}}>
      <table style={{width:'100%',borderCollapse:'collapse',fontSize:13,minWidth:900}}>
        <thead><tr>{['Cliente','Ruta','Fecha','Capital','Interes (40%)','Abonado','Saldo','Atrasado','Llevadas','Pagas','Progreso',''].map((t,i)=><th key={i} style={{textAlign:'left',padding:'7px 9px',fontSize:11,color:MT,fontWeight:700,borderBottom:'2px solid '+BD,whiteSpace:'nowrap'}}>{t}</th>)}</tr></thead>
        <tbody>{lista.map(p=>{const d=totalDeuda(p),ab=totalAbonado(p),sl=saldoTotal(p),at=saldoAtrasado(p);const pct=Math.round(ab/d*100);const term=isTerminado(p);const nSlots=Math.max(10,getAbonos(p).length);const fs=fechasCuotas(p);const h=HOY();return <tr key={p.id} style={{borderBottom:'1px solid '+BD,background:!term&&at>0.5?DNB+'44':undefined}}>
          <td style={{padding:'8px 9px'}}><div style={{fontWeight:600,cursor:'pointer',color:G,textDecoration:'underline'}} onClick={()=>onVerCliente(p)}>{p.nombre}</div>{term&&<span style={{display:'inline-block',padding:'2px 7px',borderRadius:5,background:OKB,color:OK,fontSize:11,fontWeight:700}}>Terminado</span>}</td>
          <td style={{padding:'8px 4px',cursor:'pointer'}} onClick={()=>onVerCliente(p)}><RTag r={p.ruta}/><CTag r={p.ruta}/></td>
          <td style={{padding:'8px 9px',fontSize:12,color:MT,cursor:'pointer'}} onClick={()=>onVerCliente(p)}>{fmtF(parseD(p.fechaPrestamo))}</td>
          <td style={{padding:'8px 9px',cursor:'pointer'}} onClick={()=>onVerCliente(p)}>{fmt(p.monto)}</td>
          <td style={{padding:'8px 9px',color:G,fontWeight:600,cursor:'pointer'}} onClick={()=>onVerCliente(p)}>{fmt(p.monto*0.4)}</td>
          <td style={{padding:'8px 9px',color:OK,fontWeight:600,cursor:'pointer'}} onClick={()=>onVerCliente(p)}>{fmt(ab)}<MBar pct={pct}/><div style={{fontSize:10,color:FT}}>{pct}%</div></td>
          <td style={{padding:'8px 9px',fontWeight:600,color:sl<0.5?FT:DN,cursor:'pointer'}} onClick={()=>onVerCliente(p)}>{fmt(sl)}</td>
          <td style={{padding:'8px 9px',fontWeight:600,color:at>0.5?DN:FT,cursor:'pointer'}} onClick={()=>onVerCliente(p)}>{at>0.5?fmt(at):'-'}</td>
          <td style={{padding:'8px 9px',textAlign:'center',fontWeight:700,color:TX}}>{cuotasLlevadas(p)}</td>
          <td style={{padding:'8px 9px',textAlign:'center',fontWeight:700,color:OK}}>{cuotasPagas(p)}</td>
          <td style={{padding:'8px 9px'}}><div style={{display:'flex',gap:2,flexWrap:'wrap'}}>{Array.from({length:nSlots},(_,i)=>{const a=getAbonos(p)[i];const m=a?a.monto:null;let bg=S2,c=FT;if(m&&m>0&&m>=p.cuota-0.5){bg=OK;c='#fff';}else if(m&&m>0){bg=WNB;c=WN;}else if(fs[i]&&fs[i]<h&&(m==null||m===0)){bg=DNB;c=DN;}return <span key={i} style={{width:14,height:14,borderRadius:2,display:'flex',alignItems:'center',justifyContent:'center',fontSize:8,fontWeight:700,background:bg,color:c}}>{i+1}</span>;})}</div></td>
...
function RiesgoView({prestamos,currentUser,onVerCliente}){
  ...
  const h=HOY();
  const rutasCob=currentUser.rol==='cobrador'?(currentUser.rutas||[currentUser.ruta]):null;
  const lista=useMemo(()=>{let l=prestamos.filter(p=>!isTerminado(p));if(rf)l=l.filter(p=>p.ruta===rf);if(rutasCob)l=l.filter(p=>rutasCob.includes(p.ruta));return l;},[prestamos,rf,currentUser]);
  const todos=useMemo(()=>prestamos.filter(p=>!isTerminado(p)),[prestamos]);
  function cuotasSinPago(p){return cuotasVencidas(p).filter(x=>{const a=getAbonos(p)[x.i];const m=a?a.monto:null;return m==null||m===0;}).length;}
  function deficitSistematico(p){const pagos=getAbonos(p).filter(a=>a&&a.monto>0).map(a=>a.monto);if(pagos.length<2)return null;const habitual=Math.round(pagos.reduce((s,m)=>s+m,0)/pagos.length);const diff=p.cuota-habitual;if(diff<=500)return null;return{habitual,diferencia:diff,veces:pagos.length,acumulado:diff*pagos.length};}
  function apellido(nombre){const p=String(nombre||'').trim().split(' ');return p.length>=2?p[p.length-1].toLowerCase():p[0].toLowerCase();}
  function capRec(p){const ab=totalAbonado(p);const d=totalDeuda(p);if(d<=0)return 0;return Math.min(p.monto,ab*(p.monto/d));}
  const alerta=useMemo(()=>lista.filter(p=>cuotasSinPago(p)>=2).sort((a,b)=>cuotasSinPago(b)-cuotasSinPago(a)),[lista]);
  ...
  const concentracion=useMemo(()=>{const g={};todos.forEach(p=>{const k=apellido(p.nombre);if(!g[k])g[k]=[];g[k].push(p);});const totCalle=todos.reduce((s,p)=>s+Math.max(0,p.monto-capRec(p)),0);return Object.entries(g).filter(([,ps])=>ps.length>1).map(([k,ps])=>{const activos=ps.filter(p=>!isTerminado(p));const capCalle=activos.reduce((s,p)=>s+Math.max(0,p.monto-capRec(p)),0);const pct=totCalle>0?Math.round(capCalle/totCalle*100):0;return{k,ps,activos,capCalle,pct};}).sort((a,b)=>b.capCalle-a.capCalle);},[todos]);
  const repetidores=useMemo(()=>{const g={};todos.forEach(p=>{const k=p.nombre.trim().toLowerCase();if(!g[k])g[k]=[];g[k].push(p);});return Object.values(g).filter(arr=>arr.length>1).sort((a,b)=>b.length-a.length);},[todos]);
  const TH={textAlign:'left',padding:'5px 8px',fontSize:10,color:MT,fontWeight:700,borderBottom:'2px solid '+BD};
  const TD=(ex)=>({padding:'7px 9px',...ex});
  const chip=(r)=><span style={{display:'inline-block',padding:'2px 7px',borderRadius:5,background:RUTAS[r].bg,color:RUTAS[r].c,fontSize:11,fontWeight:700}}>Ruta {r}</span>;
  return <div>
    <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:14,flexWrap:'wrap',gap:8}}>
      <div style={{fontSize:20,fontWeight:700}}>Riesgo y moras</div>
      <div style={{display:'flex',gap:6,flexWrap:'wrap'}}>{[{v:'',l:'Todas'},{v:'A',l:'Ruta A'},{v:'B',l:'Ruta B'},{v:'C',l:'Ruta C'}].map(x=><button key={x.v} onClick={()=>setRf(x.v)} style={{padding:'4px 11px',border:'1px solid '+(rf===x.v?G:BDS),borderRadius:7,background:rf===x.v?G:SF,color:rf===x.v?'#fff':MT,cursor:'pointer',fontSize:12,fontFamily:'inherit',fontWeight:rf===x.v?700:400}}>{x.l}</button>)}</div>
    </div>
    <div style={{background:SF,border:'1px solid '+BD,borderRadius:11,padding:'14px 16px',marginBottom:12}}>
      <div style={{fontSize:14,fontWeight:700,marginBottom:10}}>Resumen por ruta</div>
      <div style={{overflowX:'auto'}}><table style={{width:'100%',borderCollapse:'collapse',fontSize:13,minWidth:600}}>
        <thead><tr>{['Ruta','Activos','Con mora','Alerta 2+','Deficit','Atrasado total'].map((t,i)=><th key={i} style={TH}>{t}</th>)}</tr></thead>
        <tbody>{['A','B','C'].map(r=>{const ps=lista.filter(p=>p.ruta===r);const mora=ps.filter(p=>cuotasSinPago(p)>=1);const alr=ps.filter(p=>cuotasSinPago(p)>=2);const def=ps.filter(p=>deficitSistematico(p));const at=ps.reduce((s,p)=>s+saldoAtrasado(p),0);return <tr key={r} style={{borderBottom:'1px solid '+BD,background:alr.length?DNB+'33':'transparent'}}><td style={TD()}>{chip(r)} <span style={{fontSize:11,color:MT}}>{RUTAS[r].cobrador}</span></td><td style={TD({fontWeight:600,color:ps.length?TX:FT})}>{ps.length}</td><td style={TD({fontWeight:600,color:mora.length?WN:FT})}>{mora.length||'-'}</td><td style={TD({fontWeight:700,color:alr.length?DN:FT})}>{alr.length||'-'}</td><td style={TD({fontWeight:600,color:def.length?WN:FT})}>{def.length||'-'}</td><td style={TD({fontWeight:700,color:at>0?DN:FT})}>{at>0?fmt(at):'-'}</td></tr>;})}</tbody>
      </table></div>
    </div>
    {alerta.length>0&&<div style={{background:SF,border:'2px solid '+DN,borderRadius:11,padding:'14px 16px',marginBottom:12}}>
      <div style={{fontSize:14,fontWeight:700,color:DN,marginBottom:10}}>Alerta: {alerta.length} cliente{alerta.length!==1?'s':''} con 2+ cuotas caidas</div>
      <div style={{overflowX:'auto'}}><table style={{width:'100%',borderCollapse:'collapse',fontSize:13,minWidth:500}}>
        <thead><tr>{['Cliente','Ruta','Cuotas sin pago','Saldo atrasado','Ultimo abono'].map((t,i)=><th key={i} style={TH}>{t}</th>)}</tr></thead>
        <tbody>{alerta.map(p=>{const abs=getAbonos(p).filter(a=>a&&a.monto>0&&a.fecha).sort((a,b)=>parseD(b.fecha)-parseD(a.fecha));const ult=abs[0];return <tr key={p.id} style={{borderBottom:'1px solid '+BD,background:DNB+'33',cursor:'pointer'}} onClick={()=>onVerCliente(p)} onMouseEnter={e=>e.currentTarget.style.background=DNB+'55'} onMouseLeave={e=>e.currentTarget.style.background=DNB+'33'}><td style={TD({fontWeight:600,color:G})}>{p.nombre}</td><td style={TD()}>{chip(p.ruta)}</td><td style={TD({fontWeight:700,color:DN,fontSize:16})}>{cuotasSinPago(p)}</td><td style={TD({fontWeight:700,color:DN})}>{fmt(saldoAtrasado(p))}</td><td style={TD({fontSize:12,color:MT})}>{ult?fmt(ult.monto)+' el '+fmtF(parseD(ult.fecha)):'sin abonos'}</td></tr>;})}</tbody>
      </table></div>
    </div>}
    {['A','B','C'].map(r=>{const ps=conSaldo.filter(p=>p.ruta===r);if(!ps.length)return null;const nSlots=(p)=>Math.max(10,getAbonos(p).length);const fs=(p)=>fechasCuotas(p);return <div key={r} style={{background:SF,border:'1px solid '+BD,borderRadius:11,padding:'14px 16px',marginBottom:12}}>
      <div style={{fontSize:14,fontWeight:700,marginBottom:10,display:'flex',alignItems:'center',gap:8,flexWrap:'wrap'}}>{chip(r)}<span>{RUTAS[r].cobrador} - {ps.length} con mora</span><span style={{marginLeft:'auto',fontWeight:700,color:DN}}>{fmt(ps.reduce((s,p)=>s+saldoAtrasado(p),0))}</span></div>
      <div style={{overflowX:'auto'}}><table style={{width:'100%',borderCollapse:'collapse',fontSize:13,minWidth:560}}>
        <thead><tr>{['Cliente','Cuotas caidas','Saldo atrasado','Saldo total','Ultimo abono','Progreso'].map((t,i)=><th key={i} style={TH}>{t}</th>)}</tr></thead>
        <tbody>{ps.map(p=>{const cc=cuotasSinPago(p);const abs=getAbonos(p).filter(a=>a&&a.monto>0&&a.fecha).sort((a,b)=>parseD(b.fecha)-parseD(a.fecha));const ult=abs[0];const fcs=fs(p);return <tr key={p.id} style={{borderBottom:'1px solid '+BD,cursor:'pointer',background:cc>=2?DNB+'22':'transparent'}} onClick={()=>onVerCliente(p)} onMouseEnter={e=>e.currentTarget.style.background=S2} onMouseLeave={e=>e.currentTarget.style.background=cc>=2?DNB+'22':'transparent'}><td style={TD({fontWeight:600,color:G})}>{p.nombre}{cc>=2&&<span style={{display:'inline-block',marginLeft:6,padding:'1px 5px',borderRadius:4,background:DNB,color:DN,fontSize:10,fontWeight:700}}>ALERTA</span>}</td><td style={TD({fontWeight:700,color:cc>=2?DN:WN,fontSize:15})}>{cc||'-'}</td><td style={TD({fontWeight:700,color:DN})}>{fmt(saldoAtrasado(p))}</td><td style={TD({fontWeight:600})}>{fmt(saldoTotal(p))}</td><td style={TD({fontSize:12,color:MT})}>{ult?fmt(ult.monto)+' '+fmtF(parseD(ult.fecha)):'sin abonos'}</td><td style={TD()}><div style={{display:'flex',gap:2,flexWrap:'wrap'}}>{Array.from({length:nSlots(p)},(_,i)=>{const a=getAbonos(p)[i];const m=a?a.monto:null;let bg=S2,c=FT;if(m&&m>0&&m>=p.cuota-0.5){bg=OK;c='#fff';}else if(m&&m>0){bg=WNB;c=WN;}else if(fcs[i]&&fcs[i]<h&&(m==null||m===0)){bg=DNB;c=DN;}return <span key={i} style={{width:15,height:15,borderRadius:3,display:'flex',alignItems:'center',justifyContent:'center',fontSize:8,fontWeight:700,background:bg,color:c}}>{i+1}</span>;})}</div></td></tr>;})}
        </tbody>
      </table></div>
    </div>;})}
    {conDeficit.length>0&&<div style={{background:SF,border:'1px solid '+BD,borderRadius:11,padding:'14px 16px',marginBottom:12}}>
      <div style={{fontSize:14,fontWeight:700,marginBottom:4}}>Deficit sistematico</div>
      <div style={{fontSize:12,color:MT,marginBottom:10}}>Clientes que pagan siempre, pero siempre menos de la cuota pactada</div>
      <div style={{overflowX:'auto'}}><table style={{width:'100%',borderCollapse:'collapse',fontSize:13,minWidth:560}}>
        <thead><tr>{['Cliente','Ruta','Cuota pactada','Paga en promedio','Diferencia','Veces','Acumulado'].map((t,i)=><th key={i} style={TH}>{t}</th>)}</tr></thead>
        <tbody>{conDeficit.map(({p,d})=><tr key={p.id} style={{borderBottom:'1px solid '+BD,cursor:'pointer'}} onClick={()=>onVerCliente(p)} onMouseEnter={e=>e.currentTarget.style.background=S2} onMouseLeave={e=>e.currentTarget.style.background='transparent'}><td style={TD({fontWeight:600,color:G})}>{p.nombre}</td><td style={TD()}>{chip(p.ruta)}</td><td style={TD({color:MT})}>{fmt(p.cuota)}</td><td style={TD({fontWeight:600})}>{fmt(d.habitual)}</td><td style={TD({fontWeight:600,color:WN})}>-{fmt(d.diferencia)}</td><td style={TD({textAlign:'center'})}>{d.veces}</td><td style={TD({fontWeight:700,color:DN})}>{fmt(d.acumulado)}</td></tr>)}</tbody>
      </table></div>
    </div>}
    {concentracion.length>0&&<div style={{background:SF,border:'1px solid '+BD,borderRadius:11,padding:'14px 16px',marginBottom:12}}>
      <div style={{fontSize:14,fontWeight:700,marginBottom:4}}>Concentracion por familia</div>
      <div style={{fontSize:12,color:MT,marginBottom:10}}>Agrupado por apellido - clientes con mas de un prestamo</div>
      <div style={{overflowX:'auto'}}><table style={{width:'100%',borderCollapse:'collapse',fontSize:13,minWidth:500}}>
        <thead><tr>{['Familia','Prestamos','Activos','Capital en calle','% del total'].map((t,i)=><th key={i} style={TH}>{t}</th>)}</tr></thead>
        <tbody>{concentracion.slice(0,15).map(({k,ps,activos,capCalle,pct})=><tr key={k} style={{borderBottom:'1px solid '+BD,background:pct>=15?WNB+'33':'transparent'}}><td style={TD({fontWeight:600})}>{k.charAt(0).toUpperCase()+k.slice(1)}<div style={{fontSize:11,color:MT,marginTop:1}}>{ps.map(p=>p.nombre).join(', ')}</div></td><td style={TD({textAlign:'center',fontWeight:600})}>{ps.length}</td><td style={TD({textAlign:'center',color:activos.length?G:FT})}>{activos.length}</td><td style={TD({fontWeight:600,color:capCalle>0?TX:FT})}>{capCalle>0?fmt(capCalle):'-'}</td><td style={TD({fontWeight:700,color:pct>=15?WN:MT})}>{capCalle>0?pct+'%':'-'}{pct>=15&&<span style={{display:'inline-block',marginLeft:5,padding:'1px 5px',borderRadius:4,background:WNB,color:WN,fontSize:10}}>alto</span>}</td></tr>)}</tbody>
      </table></div>
    </div>}
    {repetidores.length>0&&<div style={{background:SF,border:'1px solid '+BD,borderRadius:11,padding:'14px 16px',marginBottom:12}}>
      <div style={{fontSize:14,fontWeight:700,marginBottom:4}}>Repetidores</div>
      <div style={{fontSize:12,color:MT,marginBottom:10}}>Clientes que han pedido mas de un prestamo</div>
      <div style={{overflowX:'auto'}}><table style={{width:'100%',borderCollapse:'collapse',fontSize:13,minWidth:500}}>
        <thead><tr>{['Cliente','Prestamos','Rutas','Capital total','Estado'].map((t,i)=><th key={i} style={TH}>{t}</th>)}</tr></thead>
        <tbody>{repetidores.map((arr,i)=>{const activo=arr.filter(p=>!isTerminado(p));const rutas=[...new Set(arr.map(p=>p.ruta))];return <tr key={i} style={{borderBottom:'1px solid '+BD,cursor:'pointer'}} onClick={()=>onVerCliente(arr[arr.length-1])} onMouseEnter={e=>e.currentTarget.style.background=S2} onMouseLeave={e=>e.currentTarget.style.background='transparent'}><td style={TD({fontWeight:600,color:G})}>{arr[0].nombre}</td><td style={TD({textAlign:'center',fontWeight:700,fontSize:16,color:G})}>{arr.length}</td><td style={TD()}>{rutas.map(r=><span key={r} style={{display:'inline-block',padding:'1px 6px',borderRadius:4,background:RUTAS[r].bg,color:RUTAS[r].c,fontSize:11,fontWeight:700,marginRight:3}}>R{r}</span>)}</td><td style={TD({fontWeight:600})}>{fmt(arr.reduce((s,p)=>s+p.monto,0))}</td><td style={TD({fontSize:12,color:activo.length?G:MT})}>{activo.length>0?activo.length+' activo'+(activo.length!==1?'s':''):'Terminados'}</td></tr>;})}
        </tbody>
      </table></div>
    </div>}
  </div>;
...
export default function App(){
  ...
  useEffect(()=>{
    ...
    const unsubS=onValue(salRef,snap=>{const val=snap.val();setSalidas(val&&Array.isArray(val)?val:val?Object.values(val):[]);setLoading(false);});
  ...
  const handleSave=useCallback((id,idx,val,fSem,fOverride,esExt)=>{
    const next=prestamos.map(p=>{
      if(p.id!==id)return p;
      const rawAb=p.abonos?(Array.isArray(p.abonos)?p.abonos:Object.values(p.abonos)):[];
      const abs=[...rawAb];
      while(abs.length<=idx)abs.push(null);
  ...
  return <div style={{display:'flex',height:'100vh',fontFamily:'-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,"Helvetica Neue",Arial,sans-serif',color:TX,background:'#F0F2F0'}}>
