...
function LoginScreen({onLogin}){
  ...
  const intentar=()=>{
    ...
    if(!u){setError('Usuario o contrasena incorrectos');return;}
    setError(''); onLogin(u);
  };
  return(
    <div style={{display:'flex',alignItems:'center',justifyContent:'center',minHeight:'100vh',background:`linear-gradient(135deg,${C.green} 0%,#0d4a26 100%)`,fontFamily:'-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Arial,sans-serif'}}>
      <div style={{background:C.surface,borderRadius:16,padding:'40px 36px',width:'100%',maxWidth:380,boxShadow:'0 24px 48px rgba(0,0,0,.25)'}}>
        <div style={{textAlign:'center',marginBottom:28}}>
          ...
          <div style={{fontSize:13,color:C.muted,marginTop:4}}>Sistema de gestion de prestamos</div>
        </div>
      ...
      <div style={{background:'#fff',borderRadius:14,width:'100%',maxWidth:700,maxHeight:'90vh',overflowY:'auto',boxShadow:'0 24px 48px rgba(0,0,0,.25)'}} onClick={e=>e.stopPropagation()}>
        <div style={{padding:'18px 22px',borderBottom:`1px solid ${C.border}`,display:'flex',justifyContent:'space-between',alignItems:'center',background:C.greenBg,borderRadius:'14px 14px 0 0'}}>
          <div>
            ...
            <div style={{fontSize:13,color:C.muted,marginTop:3,display:'flex',alignItems:'center',gap:6}}><RTag r={p.ruta}/><CobTag r={p.ruta}/>{p.tel&&<span>Tel: {p.tel}</span>}</div>
          </div>
        ...
        return <Panel key={k} title={<span><RTag r={k}/><CobTag r={k}/><span style={{fontSize:12,color:C.muted,marginLeft:7}}>{RUTAS[k].dia} . {xs.length} cobros</span></span>} tr={<span style={{fontSize:12,color:C.muted}}>Recogido <b style={{color:C.ok}}>{fmt(rec)}</b> de {fmt(esp)} . falta <b style={{color:C.danger}}>{fmt(esp-rec)}</b></span>}>
          <div style={{overflowX:'auto'}}>
            <table style={{width:'100%',borderCollapse:'collapse',fontSize:13,minWidth:660}}>
              <thead><tr>{['Cliente','Cuota','Vence','Pactado','Abono','Saldo','Rapido'].map((h,i)=><th key={i} style={{textAlign:'left',padding:'6px 8px',fontSize:11,color:C.muted,fontWeight:700,borderBottom:`2px solid ${C.border}`}}>{h}</th>)}</tr></thead>
              <tbody>{xs.map((x,idx)=>{
                ...
                return <tr key={idx} style={{borderBottom:`1px solid ${C.border}`,background:rowBg}}>
                  ...
                  <td style={{padding:'8px',fontWeight:700,color:saldo<0.5?C.ok:C.danger}}>{saldo<0.5?'OK':fmt(saldo)}</td>
                  <td style={{padding:'8px'}}>
                    <div style={{display:'flex',gap:3}}>
                      <Btn sm v="primary" onClick={()=>onSave(x.p.id,x.i,'completo',toDS(x.f),null,x.ext)}>OK</Btn>
                      {!x.ext&&<Btn sm style={{color:C.warn,borderColor:C.warnBg}} onClick={()=>onSave(x.p.id,x.i,0,toDS(x.f))}>X</Btn>}
  ...
  return(
    <div>
      ...
      <div style={{display:'flex',gap:8,marginBottom:12,flexWrap:'wrap',alignItems:'flex-end'}}>
        ...
        <span style={{fontSize:12,color:C.faint}}>{lista.length} prestamo{lista.length!==1?'s':''}</span>
      </div>
...
function RiesgoView({prestamos,currentUser}){
  ...
  return(
    <div>
      ...
      <Panel title="Resumen por ruta">
        <div style={{overflowX:'auto'}}><table style={{width:'100%',borderCollapse:'collapse',fontSize:13,minWidth:560}}><thead><tr>{['Ruta','Activos','En mora','Alerta (3+)','Con deficit','Atrasado'].map((h,i)=><th key={i} style={{textAlign:'left',padding:'6px 8px',fontSize:11,color:C.muted,fontWeight:700,borderBottom:`2px solid ${C.border}`}}>{h}</th>)}</tr></thead>
      ...
      <Panel title="Clientes con saldo atrasado">
        {conSaldo.length===0?<Empty msg="Nadie tiene saldo atrasado."/>:
          <div style={{overflowX:'auto'}}><table style={{width:'100%',borderCollapse:'collapse',fontSize:13,minWidth:600}}><thead><tr>{['Cliente','Ruta','Cuotas con saldo','Atrasado','Ultimo abono','Saldo total'].map((h,i)=><th key={i} style={{textAlign:'left',padding:'5px 8px',fontSize:11,color:C.muted,fontWeight:700,borderBottom:`2px solid ${C.border}`}}>{h}</th>)}</tr></thead>
      ...
      {conDeficit.length>0&&<Panel title="Deficit sistematico (pagan siempre menos de lo pactado)">
    ...
    <div>
      ...
      <Panel title="">
        <div style={{maxWidth:520}}>
          ...
          {monto&&<div style={{padding:'10px 13px',background:C.s2,borderRadius:8,fontSize:13,color:C.muted,marginBottom:12}}>Capital: <b>{fmt(parseFloat(monto))}</b> . Interes 40%: <b>{fmt(parseFloat(monto)*0.4)}</b> . Total a recibir: <b style={{color:C.green}}>{fmt(parseFloat(monto)*1.4)}</b></div>}
    ...
    <div>
      ...
      <Panel title="Rentabilidad real">
        <div style={{background:C.greenBg,border:`1px solid ${C.green}22`,borderRadius:8,padding:14,marginBottom:14}}>
          {[{l:'Interes cobrado (ganancia bruta)',v:rr.intCob,c:C.ok},{l:' -  Salidas de caja',v:rr.sal,c:C.danger},{l:'= Ganancia neta',v:rr.ganancia,c:rr.ganancia>0?C.ok:C.danger,big:true},{l:`Capital en la calle (activos)`,v:rr.capCalle,c:C.warn},{l:'+ Interes por cobrar',v:rr.intPend,c:C.muted},{l:'= Total si todos pagan',v:rr.capCalle+rr.intPend,c:C.text,big:true}].map((row,i)=>
      ...
      <Panel title="Registrar salida de caja">
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(150px,1fr))',gap:12,maxWidth:640}}>
          ...
          <Inp label="Concepto / a quien" value={sq} onChange={e=>setSq(e.target.value)} placeholder="Jhon, semana del 13"/>
        </div>
      ...
      <Panel title={`Salidas  -  ${fmt(salidas.reduce((s,x)=>s+x.monto,0))}`}>
        {salidasOrd.length===0?<Empty msg="Sin salidas registradas."/>:
          <div style={{overflowX:'auto'}}><table style={{width:'100%',borderCollapse:'collapse',fontSize:13,minWidth:480}}><thead><tr>{['Fecha','Categoria','Concepto','Monto',''].map((h,i)=><th key={i} style={{textAlign:'left',padding:'5px 8px',fontSize:11,color:C.muted,fontWeight:700,borderBottom:`2px solid ${C.border}`}}>{h}</th>)}</tr></thead>
...
function RespaldoView({prestamos,salidas,onRestaurar}){
  ...
  const importar=e=>{const f=e.target.files?.[0];if(!f)return;const r=new FileReader();r.onload=ev=>{try{const d=JSON.parse(String(ev.target?.result));if(!d.prestamos||!Array.isArray(d.prestamos))return alert('Archivo invalido');if(!window.confirm(`?Cargar ${d.prestamos.length} prestamos?`))return;onRestaurar(d);}catch{alert('No se pudo leer el archivo.');}};r.readAsText(f);e.target.value='';};
  return(
    <div>
...
export default function App(){
  ...
  const saveP=useCallback(async next=>{ setPrestamos(next); try{ await set(ref(db,'prestamos'),next); }catch{ notify('Error al guardar. Verifica tu conexion.',true); } },[]);
  ...
  return(
    <div style={{display:'flex',height:'100vh',fontFamily:'-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,"Helvetica Neue",Arial,sans-serif',color:C.text,background:C.bg,WebkitFontSmoothing:'antialiased'}}>
      <nav style={{width:204,minWidth:204,background:C.surface,borderRight:`1px solid ${C.border}`,display:'flex',flexDirection:'column',padding:'10px 0',height:'100vh',overflowY:'auto'}}>
        ...
        <button onClick={()=>setCurrentUser(null)} style={{display:'flex',alignItems:'center',gap:7,padding:'10px 13px',background:'transparent',color:C.danger,border:'none',cursor:'pointer',fontSize:13,textAlign:'left',width:'100%',fontFamily:'inherit',borderTop:`1px solid ${C.border}`,marginTop:4}}> Cerrar sesion</button>
        {!syncOk&&<div style={{padding:'8px 13px',fontSize:11,color:C.warn,background:C.warnBg}}> Sin conexion a la base de datos</div>}
      </nav>
