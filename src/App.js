...
function ModalSemana({lun,dom,prestamos,onClose,onVerCliente}){
  if(!lun)return null;
  const [rutaTab,setRutaTab]=useState('todas');
...
function Dashboard({prestamos,flujoOff,setFlujoOff,onVerCliente}){
  ...
  return <div>
    {mSemana&&<ModalSemana lun={mSemana.lun} dom={mSemana.dom} prestamos={prestamos} onClose={()=>setMSemana(null)} onVerCliente={onVerCliente}/>}
