            ...
            return <tr key={idx} style={{borderBottom:'1px solid '+BD,background:rowBg}}>
              ...
              <td style={{padding:'8px'}}>
                <input
                  ...
                  placeholder="monto"
                  min="0"
                  value={inputVals[x.p.id+'-'+x.i]??( m==null?'':String(Math.round(m)))}
                  ...
                  onChange={ev=>{
                    ...
                    setInputVals(prev=>({...prev,[x.p.id+'-'+x.i]:val}));
                    onSave(x.p.id,x.i,val,toDS(x.f));
                  }}
                  onBlur={ev=>{
                    // Limpiar del estado local cuando pierde foco (Firebase ya tiene el valor)
