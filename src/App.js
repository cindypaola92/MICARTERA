...
function Dashboard({prestamos,salidas,flujoOff,setFlujoOff,onVerCliente}){
  ...
  const coEsta=[];activos.forEach(p=>{const fs=fechasCuotas(p);let tieneEsta=false;fs.forEach((f,i)=>{if(enRango(f,lh,dh)){coEsta.push({p,i});tieneEsta=true;}});if(!tieneEsta&&saldoTotal(p)>0.5&&fs[9]<lh){coEsta.push({p,i:getAbonos(p).length,ext:true});}});
  ...
  return <div>
    ...
    {lista.length===0?<Empty msg="Sin resultados."/>:<div style={{background:SF,border:'1px solid '+BD,borderRadius:10,overflowX:'auto',marginBottom:12}}>
      <table style={{width:'100%',borderCollapse:'collapse',fontSize:13,minWidth:780}}>
        <thead><tr>{['Cliente','Ruta','Fecha','Capital','Interes (40%)','Abonado','Saldo','Atrasado','Cuotas',''].map((t,i)=><th key={i} style={{textAlign:'left',padding:'7px 9px',fontSize:11,color:MT,fontWeight:700,borderBottom:'2px solid '+BD,whiteSpace:'nowrap'}}>{t}</th>)}</tr></thead>
