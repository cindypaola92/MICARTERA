  ...
  return <Overlay onClose={onClose}>
    ...
    <div style={{padding:'14px 18px'}}>
      ...
      {recs.length>0&&<div style={{marginBottom:16}}>
        <div style={{fontSize:13,fontWeight:700,marginBottom:8,color:TX}}>Cobros recibidos</div>
        <div style={{overflowX:'auto'}}><table style={{width:'100%',borderCollapse:'collapse',fontSize:13,minWidth:480}}>
          ...
          <tbody>{recs.map((x,i)=><tr key={i} style={{borderBottom:'1px solid '+BD,background:i%2===0?'transparent':S2}}>
            ...
            <td style={{padding:'7px 8px',fontWeight:600}}>{x.i+1>10?'Ext':'C'+(x.i+1)}</td>
            <td style={{padding:'7px 8px',fontWeight:700,color:OK}}>{fmt(x.monto)}</td>
          </tr>)}</tbody>
        </table></div>
      </div>}
      {nuevos.length>0&&<div>
        <div style={{fontSize:13,fontWeight:700,marginBottom:8,color:TX}}>Prestamos nuevos esta semana</div>
...
function Dashboard({prestamos,flujoOff,setFlujoOff,onVerCliente}){
  ...
  return <div>
    ...
    <Panel title={'Historial - ultimas '+HIST+' semanas'} tr={<span style={{fontSize:11,color:G,fontWeight:600}}>Clic en una celda para ver detalle de esa ruta</span>}>
      <div style={{overflowX:'auto'}}><table style={{width:'100%',borderCollapse:'collapse',fontSize:12,minWidth:580}}>
        <thead><tr>
          <th style={{textAlign:'left',padding:'5px 7px',fontSize:10,color:MT,fontWeight:700,borderBottom:'2px solid '+BD}}>Semana</th>
          {['A','B','C'].map(k=><th key={k} style={{padding:'5px 7px',fontSize:10,color:RUTAS[k].c,fontWeight:700,borderBottom:'2px solid '+BD,textAlign:'center',borderLeft:'1px solid '+BD}}>Ruta {k} - {RUTAS[k].cobrador}<div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:4,marginTop:3}}><span style={{fontSize:9,color:OK,fontWeight:600}}>Recogido</span><span style={{fontSize:9,color:MT,fontWeight:600}}>Prestado</span></div></th>)}
          <th style={{padding:'5px 7px',fontSize:10,color:MT,fontWeight:700,borderBottom:'2px solid '+BD,textAlign:'center'}}>Total</th>
          <th style={{padding:'5px 7px',fontSize:10,color:MT,fontWeight:700,borderBottom:'2px solid '+BD,textAlign:'center'}}>Neto</th>
        </tr></thead>
        <tbody>{hist.map((x,idx)=>{
          ...
          return <tr key={idx} style={{borderBottom:'1px solid '+BD,background:esA?GB:'transparent'}}>
            <td style={{padding:'7px',fontSize:11,color:esA?G:MT,fontWeight:esA?700:400,cursor:'pointer'}} onClick={()=>setMSemana({lun:x.lun,dom:x.dom,ruta:'todas'})}>{x.lun.getDate()+' '+MESES[x.lun.getMonth()]+' - '+x.dom.getDate()+' '+MESES[x.dom.getMonth()]}{esA&&<div style={{fontSize:10,color:G}}>esta semana</div>}</td>
            {['A','B','C'].map(k=><td key={k} style={{padding:'7px 5px',textAlign:'center',cursor:'pointer',borderLeft:'1px solid '+BD}} onClick={()=>setMSemana({lun:x.lun,dom:x.dom,ruta:k})} onMouseEnter={e=>e.currentTarget.style.background=RUTAS[k].bg} onMouseLeave={e=>e.currentTarget.style.background='transparent'}>
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:4}}>
                <div style={{fontWeight:700,color:x.f[k].rec?OK:FT,fontSize:12}}>{x.f[k].rec?fmt(x.f[k].rec):'-'}</div>
                <div style={{fontWeight:600,color:x.f[k].cap?TX:FT,fontSize:12}}>{x.f[k].cap?fmt(x.f[k].cap):'-'}</div>
              </div>
              {x.f[k].nPr>0&&<div style={{fontSize:10,color:RUTAS[k].c,marginTop:2}}>{x.f[k].nPr} nuevo{x.f[k].nPr!==1?'s':''}</div>}
            </td>)}
            <td style={{padding:'7px 5px',fontWeight:700,color:tR?OK:FT,textAlign:'center',cursor:'pointer'}} onClick={()=>setMSemana({lun:x.lun,dom:x.dom,ruta:'todas'})}>{tR?fmt(tR):'-'}</td>
            <td style={{padding:'7px 5px',fontWeight:700,color:n>0?OK:n<0?DN:FT,textAlign:'center'}}>{(tR||tP)?(n>0?'+':'')+fmt(n):'-'}</td>
          </tr>;
        })}</tbody>
      </table></div>
    </Panel>
