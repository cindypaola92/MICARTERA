            ...
            return <tr key={idx} style={{borderBottom:'1px solid '+BD,background:rowBg}}>
              ...
              <td style={{padding:'8px'}}>
                <input
                  ...
                  style={{width:86,padding:'4px 6px',fontSize:12,border:'1px solid '+(inputVals[x.p.id+'-'+x.i]!==undefined?G:BDS),borderRadius:6,background:SF,color:TX,fontFamily:'inherit'}}
                  onChange={ev=>{
                  ...
                  onKeyDown={ev=>{
                    if(ev.key==='Enter'){
                      ...
                      if(val!==''&&!isNaN(n)&&n>=0){
                        ...
                        setInputVals(prev=>{const nx={...prev};delete nx[x.p.id+'-'+x.i];return nx;});
                      }
                      ev.target.blur();
                    }
                  }}
...
function ConsolidadoView({prestamos,onEdit,onDelete,currentUser,onVerCliente}){
  ...
  const lista=useMemo(()=>{
    ...
    if(filtroCuotasPagas!==''){
      ...
      l=l.filter(p=>{
        const pagadas=cuotasPagadasCualquiera(p);
        return pagadas===n;
      });
    }
