  ...
  return <div>
    ...
    {lista.length===0?<Empty msg="Sin resultados."/>:<div style={{background:SF,border:'1px solid '+BD,borderRadius:10,overflowX:'auto',marginBottom:12}}>
      <table style={{width:'100%',borderCollapse:'collapse',fontSize:13,minWidth:820}}>
        <thead><tr>{['Cliente','Ruta','Fecha','Capital','Abonado','Saldo','Atrasado','Cuotas vencidas / pagas',''].map((t,i)=><th key={i} style={{textAlign:'left',padding:'7px 9px',fontSize:11,color:MT,fontWeight:700,borderBottom:'2px solid '+BD,whiteSpace:'nowrap'}}>{t}</th>)}</tr></thead>
        <tbody>{lista.map(p=>{
...
function CarteraActivaView({prestamos,salidas,onVerCliente}){
  ...
  return <div>
    ...
    <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(130px,1fr))',gap:10,marginBottom:14}}>
      <KPI label="Clientes activos" value={String(activos.length)} sub="prestamos vigentes" vc={G} onClick={()=>setTab('activos')}/>
      <KPI label="Capital en la calle" value={fmt(totA.cap)} sub="sin interes" onClick={()=>setTab('activos')}/>
      <KPI label="Por cobrar (con interes)" value={fmt(totA.saldo)} vc={G} onClick={()=>setTab('activos')}/>
      <KPI label="Saldo atrasado" value={fmt(totA.atras)} vc={totA.atras>0?DN:FT} sub={activos.filter(tieneSaldoAtrasado).length+' clientes'} onClick={()=>setTab('activos')}/>
      <KPI label="Terminados" value={String(terminados.length)} sub={fmt(totT.cobrado)+' cobrado'} vc={OK} onClick={()=>setTab('terminados')}/>
      <KPI label="Ganancia neta" value={fmt(gananciaTotal)} vc={gananciaTotal>0?OK:DN}/>
    </div>
    ...
    <div style={{display:'flex',gap:0,marginBottom:0,borderBottom:'2px solid '+BD}}>
      {[{v:'activos',l:'Activos ('+activos.length+')'},{v:'terminados',l:'Terminados ('+terminados.length+')'},{v:'todos',l:'Todos ('+prestamos.length+')'}].map(t=><button key={t.v} onClick={()=>setTab(t.v)} style={{padding:'8px 18px',border:'none',borderBottom:tab===t.v?'2px solid '+G:'2px solid transparent',background:'none',cursor:'pointer',fontSize:13,fontWeight:tab===t.v?700:400,color:tab===t.v?G:MT,fontFamily:'inherit',marginBottom:-2}}>{t.l}</button>)}
    </div>
    ...
    <div style={{background:SF,border:'1px solid '+BD,borderTop:'none',borderRadius:'0 0 10px 10px',overflowX:'auto',marginBottom:12}}>
      <table style={{width:'100%',borderCollapse:'collapse',fontSize:13,minWidth:820}}>
        ...
        <tbody>{lista.sort((a,b)=>parseD(b.fechaPrestamo)-parseD(a.fechaPrestamo)).map(p=>{
          const d=totalDeuda(p),ab=totalAbonado(p),sl=saldoTotal(p),at=saldoAtrasado(p);
          const pct=Math.round(ab/d*100);
          const term=isTerminado(p);
          const cTot=cuotasTotalesVencidas(p);
    ...
    <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(130px,1fr))',gap:9}}>
      <KPI label="Capital total" value={fmt(prestamos.reduce((s,p)=>s+p.monto,0))} sub={prestamos.length+' prestamos'}/>
      <KPI label="Total abonado" value={fmt(prestamos.reduce((s,p)=>s+totalAbonado(p),0))} vc={OK}/>
      <KPI label="Por cobrar" value={fmt(activos.reduce((s,p)=>s+saldoTotal(p),0))} vc={G}/>
      <KPI label="Salidas de caja" value={fmt(totalSalidas)} vc={DN}/>
      <KPI label="Ganancia neta" value={fmt(gananciaTotal)} vc={gananciaTotal>0?OK:DN}/>
    </div>
  </div>;
}
