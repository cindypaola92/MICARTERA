...
function Dashboard({prestamos,salidas,flujoOff,setFlujoOff,onVerCliente}){
  ...
  const coEsta=[];activos.forEach(p=>{
    fechasCuotas(p).forEach((f,i)=>{if(enRango(f,lh,dh))coEsta.push({p,i,ext:false});});
    // Clientes con plan vencido y saldo pendiente
    const fs=fechasCuotas(p);
    const tienePlan=fs.some(f=>enRango(f,lh,dh));
    if(!tienePlan&&saldoTotal(p)>0.5&&fs[9]<=dh){
      const abs=getAbonos(p);
      const abEsta=abs.some(a=>a&&a.fecha&&enRango(parseD(a.fecha),lh,dh));
      if(!abEsta)coEsta.push({p,i:abs.length,ext:true,pendiente:true});
    }
  });
    ...
    <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
      <Panel title="Cobros esta semana">
        {['A','B','C'].map(k=>{
          ...
          const normales=xs.filter(x=>!x.pendiente);
          const pendientes=xs.filter(x=>x.pendiente);
          const e2=normales.reduce((s,x)=>s+x.p.cuota,0);
          const rc=xs.reduce((s,x)=>s+(getAbonos(x.p)[x.i]?(getAbonos(x.p)[x.i].monto||0):0),0);
          return <div key={k} style={{marginBottom:11}}>
            <div style={{display:'flex',justifyContent:'space-between',marginBottom:6}}>
              <span><RTag r={k}/><CTag r={k}/></span>
              <span style={{fontSize:12}}><b style={{color:OK}}>{fmt(rc)}</b> / {fmt(e2)}{pendientes.length>0&&<span style={{color:DN,marginLeft:6}}>+{pendientes.length} venc.</span>}</span>
            </div>
            {normales.map((x,i)=>{const m=getAbonos(x.p)[x.i]?getAbonos(x.p)[x.i].monto:null;return <div key={i} onClick={()=>onVerCliente(x.p)} style={{fontSize:12,padding:'4px 6px',cursor:'pointer',borderRadius:4,display:'flex',justifyContent:'space-between',alignItems:'center',borderBottom:'1px solid '+BD+'44'}} onMouseEnter={e=>e.currentTarget.style.background=S2} onMouseLeave={e=>e.currentTarget.style.background='transparent'}><span style={{color:TX}}>{x.p.nombre} - C{x.i+1}</span><span style={{fontWeight:700,color:m&&m>0?OK:m===0?DN:FT,fontSize:11,marginLeft:8}}>{m!=null?fmt(m):'sin registrar'}</span></div>;})}
            {pendientes.length>0&&<div style={{marginTop:6,paddingTop:6,borderTop:'1px dashed '+DN+'44'}}>
              <div style={{fontSize:10,fontWeight:700,color:DN,marginBottom:4,textTransform:'uppercase'}}>Plan vencido - saldo pendiente</div>
              {pendientes.map((x,i)=>{const m=getAbonos(x.p)[x.i]?getAbonos(x.p)[x.i].monto:null;return <div key={i} onClick={()=>onVerCliente(x.p)} style={{fontSize:12,padding:'4px 6px',cursor:'pointer',borderRadius:4,display:'flex',justifyContent:'space-between',alignItems:'center',borderBottom:'1px solid '+DNB,background:DNB+'44'}} onMouseEnter={e=>e.currentTarget.style.background=DNB} onMouseLeave={e=>e.currentTarget.style.background=DNB+'44'}><span style={{color:DN,fontWeight:600}}>{x.p.nombre} <span style={{fontWeight:400,fontSize:10}}>C{x.i+1}★</span></span><span style={{fontWeight:700,color:DN,fontSize:11,marginLeft:8}}>{fmt(saldoTotal(x.p))}</span></div>;})}
            </div>}
          </div>;
        })}
...
function CobrosView({prestamos,onSave,semOff,setSemOff,currentUser,onVerCliente}){
  ...
  const [inputVals,setInputVals]=useState({});  // {`id-i`: valor local del input}
  // Sincronizar inputVals cuando cambia semana o prestamos
  const prevSemOff=useRef(semOff);
  useEffect(()=>{if(prevSemOff.current!==semOff){setInputVals({});prevSemOff.current=semOff;}},[semOff]);
  ...
  const cobros=useMemo(()=>{
    ...
    prestamos.filter(p=>!isTerminado(p)).forEach(p=>{
      ...
      // Si no hay cuota del plan esta semana y el plan vencio con saldo pendiente, mostrar como pendiente
      // Condición: cuota 10 ya venció (antes o durante esta semana) y tiene saldo
      if(!tienePlanEsta&&saldoTotal(p)>0.5&&cuotas[cuotas.length-1]<=dom){
            ...
            return <tr key={idx} style={{borderBottom:'1px solid '+BD,background:rowBg}}>
              ...
              <td style={{padding:'8px'}}>
                <input
                  ...
                  placeholder="monto"
                  min="0"
                  value={inputVals[x.p.id+'-'+x.i]!==undefined?inputVals[x.p.id+'-'+x.i]:(m==null?'':String(Math.round(m)))}
                  style={{width:86,padding:'4px 6px',fontSize:12,border:'1px solid '+BDS,borderRadius:6,background:SF,color:TX,fontFamily:'inherit'}}
                  onChange={ev=>{
                    // Solo actualiza el estado local, NO guarda en Firebase todavía
                    setInputVals(prev=>({...prev,[x.p.id+'-'+x.i]:ev.target.value}));
                  }}
                  onBlur={ev=>{
                    // Guarda en Firebase solo cuando el usuario termina de escribir
                    const val=ev.target.value;
                    if(val!==''&&val!==String(m==null?'':Math.round(m))){
                      onSave(x.p.id,x.i,val,toDS(x.f));
                    }
                    setInputVals(prev=>{const n={...prev};delete n[x.p.id+'-'+x.i];return n;});
                  }}
                  onKeyDown={ev=>{
                    // Guardar también al presionar Enter
                    if(ev.key==='Enter'){
                      const val=ev.target.value;
                      if(val!=='')onSave(x.p.id,x.i,val,toDS(x.f));
                      ev.target.blur();
                    }
                  }}
                />
                {m!=null&&<input type="date" defaultValue={fa||toDS(x.f)} key={'d'+x.p.id+'-'+x.i+'-'+fa} style={{width:118,padding:'3px 5px',fontSize:11,border:'1px solid '+BDS,borderRadius:6,background:SF,color:TX,fontFamily:'inherit',marginTop:3,display:'block'}} onChange={ev=>onSave(x.p.id,x.i,null,null,ev.target.value)}/>}
...
function cuotasPagadasCompletas(p){
  // Solo completas (pagó el monto exacto o más)
  const nS=cuotasTotalesReales(p);
  return Array.from({length:nS},(_,i)=>estadoCuota(p,i)).filter(e=>e==='completa').length;
}
function cuotasPagadasCualquiera(p){
  // Completas + parciales (cualquier pago registrado)
  const nS=cuotasTotalesReales(p);
  return Array.from({length:nS},(_,i)=>estadoCuota(p,i)).filter(e=>e==='completa'||e==='parcial').length;
}
...
function ConsolidadoView({prestamos,onEdit,onDelete,currentUser,onVerCliente}){
  ...
  const [tipoPagas,setTipoPagas]=useState('cualquiera'); // 'cualquiera' | 'completas'
  const [operadorPagas,setOperadorPagas]=useState('min'); // 'min' | 'exacto'
  ...
  const lista=useMemo(()=>{
    ...
    if(filtroCuotasPagas!==''){
      const n=parseInt(filtroCuotasPagas);
      l=l.filter(p=>{
        const pagadas=tipoPagas==='completas'?cuotasPagadasCompletas(p):cuotasPagadasCualquiera(p);
        return operadorPagas==='min'?pagadas>=n:pagadas===n;
      });
    }
    return l.sort((a,b)=>parseD(b.fechaPrestamo)-parseD(a.fechaPrestamo));
  },[prestamos,rf,estado,nombre,currentUser,filtroCuotasTotal,filtroCuotasPagas,tipoPagas,operadorPagas]);
  ...
  return <div>
    ...
    <div style={{background:S2,padding:'12px 14px',borderRadius:8,border:'1px solid '+BD,marginBottom:12}}>
      <div style={{fontSize:12,fontWeight:700,color:MT,marginBottom:10}}>Filtrar por cuotas</div>
      <div style={{display:'flex',gap:8,flexWrap:'wrap',alignItems:'center'}}>
        <span style={{fontSize:12,color:MT}}>Clientes que llevan</span>
        <Sel value={filtroCuotasTotal} onChange={e=>setFiltroCuotasTotal(e.target.value)} style={{maxWidth:70}}>
          <option value="">todas</option>
          {opcionesCuotasTotal.map(n=><option key={n} value={n}>{n}</option>)}
        </Sel>
        <span style={{fontSize:12,color:MT}}>cuotas vencidas y han pagado</span>
        <Sel value={filtroCuotasPagas} onChange={e=>setFiltroCuotasPagas(e.target.value)} style={{maxWidth:70}}>
          <option value="">todas</option>
          {Array.from({length:13},(_,i)=>i+1).map(n=><option key={n} value={n}>{n}</option>)}
        </Sel>
        <span style={{fontSize:12,color:MT}}>(completas o parciales)</span>
        {(filtroCuotasTotal!==''||filtroCuotasPagas!=='')&&
          <Btn sm onClick={()=>{setFiltroCuotasTotal('');setFiltroCuotasPagas('');setTipoPagas('cualquiera');setOperadorPagas('min');}}>Limpiar</Btn>}
        <span style={{fontSize:13,fontWeight:700,color:G,padding:'4px 10px',background:GB,borderRadius:6,border:'1px solid '+G+'33',marginLeft:4}}>
          {lista.length} cliente{lista.length!==1?'s':''}
        </span>
      </div>
      {filtroCuotasTotal!==''&&filtroCuotasPagas!==''&&
        <div style={{fontSize:11,color:G,marginTop:8,fontWeight:600}}>
          Mostrando: clientes con {filtroCuotasTotal} cuotas vencidas que han pagado {filtroCuotasPagas} o más
        </div>}
    </div>
        ...
        <tbody>{lista.map(p=>{
          ...
          return <tr key={p.id} style={{borderBottom:'1px solid '+BD,background:!term&&at>0.5?DNB+'44':undefined,cursor:'pointer'}} onClick={()=>onVerCliente(p)} onMouseEnter={e=>e.currentTarget.style.background=!term&&at>0.5?DNB+'66':S2} onMouseLeave={e=>e.currentTarget.style.background=!term&&at>0.5?DNB+'44':'transparent'}>
            ...
            <td style={{padding:'8px 9px'}}>
              {(()=>{const cCual=cuotasPagadasCualquiera(p);const cComp=cuotasPagadasCompletas(p);const cParc=cCual-cComp;return(
              <div style={{display:'flex',alignItems:'center',gap:4,marginBottom:4,flexWrap:'wrap'}}>
                <span style={{fontSize:12,fontWeight:700,color:TX}}>{cTot} venc.</span>
                <span style={{fontSize:12,color:MT}}>/</span>
                <span style={{fontSize:12,fontWeight:700,color:cComp===cTot&&cTot>0?OK:cComp>0?G:DN}}>{cComp} compl.</span>
                {cParc>0&&<span style={{fontSize:11,fontWeight:700,color:WN,background:WNB,padding:'1px 6px',borderRadius:4}}>{cParc} parc.</span>}
...
export default function App(){
  ...
  const saveP=useCallback(async next=>{
    // Limpiar NaN/undefined antes de enviar a Firebase
    const clean=JSON.parse(JSON.stringify(next,(k,v)=>v!==undefined&&!Number.isNaN(v)?v:null));
    setPrestamos(clean);
    try{await set(ref(db,'prestamos'),clean);}
    catch(e){console.error('Error Firebase:',e);notify('Error al guardar. Verifica tu conexión.',true);}
  },[]);
  const saveS=useCallback(async next=>{
    const clean=JSON.parse(JSON.stringify(next,(k,v)=>v!==undefined&&!Number.isNaN(v)?v:null));
    setSalidas(clean);
    try{await set(ref(db,'salidas'),clean);}
    catch(e){console.error('Error Firebase:',e);notify('Error al guardar. Verifica tu conexión.',true);}
  },[]);
  ...
  const handleSave=useCallback((id,idx,val,fSem,fOverride,esExt)=>{
    // Validar antes de tocar Firebase
    if(val===''||val===undefined)return;
    const numVal=parseFloat(val);
    const esNumerico=val!=='limpiar'&&val!=='completo'&&val!==0&&val!=='0'&&fOverride===null;
    if(esNumerico&&(isNaN(numVal)||numVal<0))return;
    const next=prestamos.map(p=>{
      ...
      else{const pv=abs[idx];abs[idx]={monto:Math.round(Math.max(0,numVal)),fecha:(pv&&pv.fecha)||fDef};}
      return{...p,abonos:abs};
    });
    saveP(next);
  },[prestamos]);
